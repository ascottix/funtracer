...
func TestSceneWithBoundingBoxes(t *testing.T) {
	...
	floor := NewPlane()
	// TODO: if the plane is not y-translated by +/-Epsilon then rendering artifacts occur on the bounding boxes
	floor.SetTransform(RotationY(0.31415), Translation(0, Epsilon, 0))
	patt1 := NewCheckerPattern(Gray(0.65), Gray(0.35))
	patt1.SetTransform(Translation(0, 0.1, 0))
	floor.Material().SetPattern(patt1)
	floor.Material().SetSpecular(0).SetReflective(0.4)
